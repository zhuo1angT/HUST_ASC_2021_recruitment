...
template <typename P>
ImageClass<P>::ImageClass(int const _width, int const _height) : width(_width), height(_height) {
  // Initialize a blank image

  pixel = (P *)malloc(sizeof(P) * width * height);

  for (int j = 0; j < width; j++)
    for (int i = 0; i < height; i++) pixel[i * width + j] = (P)0;
}
...
ImageClass<P>::ImageClass(char const *file_name) {
  ...
  for (int i = 0; i < height; i++) row[i] = (png_byte *)malloc(png_get_rowbytes(ptr, info));
  ...
  for (int j = 0; j < width; j++)
    for (int i = 0; i < height; i++) pixel[i * width + j] = (P)row[i][j];

  for (int i = 0; i < height; i++) free(row[i]);
  free(row);
}
...
void ImageClass<P>::WriteToFile(char const *file_name) {
  ...
  png_set_IHDR(ptr, info, width, height, 8, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE,
